...
def create_adf_mask(array_shape,radii):
    [r0,r1]=radii
    ...
    adf_mask = np.array(kdist <= r1, np.int)*np.array(kdist >= r0, np.int)
    return adf_mask

def adf_image(data4d,radii,*args):
    
    data4d = np.moveaxis(data4d, [0,1,2,3], [2,3,0,1]) # swap axes to get real space coordinates first
    data4d = ( data4d - data4d.min() ) / ( data4d.max() - data4d.min() ) # normalising data
    data4d = data4d + 1e-10 # to avoid log zero errors
    dim =data4d.shape #N_x1,N_x2,N_k1,N_k2
    
    adf_mask=create_adf_mask((dim[2],dim[3]),radii)
    adf=np.mean(data4d*adf_mask,axis=(-2,-1))    
    adf_bndry=np.logical_xor(ndimage.binary_dilation(adf_mask),adf_mask)

    mean_log_dp = np.log( np.mean(data4d, axis = (0,1)) )
    mean_log_dp_norm = ( mean_log_dp - mean_log_dp.min() ) / ( mean_log_dp.max() - mean_log_dp.min() ) 
    img=plt.cm.viridis(mean_log_dp_norm)#use grayscale colormap

    img[adf_bndry]=[1,0,0,1] #make the boundary red
    
    fig=plt.figure();ax1=fig.add_subplot(1,2,1);
    ax1.imshow(adf, cmap = 'viridis');ax1.axis('off')
